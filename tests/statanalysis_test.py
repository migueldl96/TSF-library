...
sys.path.append('../tsf')
from tsf.windows.dinamic import StatAnalysis
...
class TestDinamicWindow(unittest.TestCase):
    ...
    def test_single_maker(self):
        ...
        sa = StatAnalysis(ratio=ratio)
        result = sa.transform(X=[], y=self.data[0])
    ...
    def test_multi_maker(self):
        ...
        sa = StatAnalysis(ratio=ratio, stat='variance', horizon=1)
        Xt = sa.fit(X=[], y=self.data).transform(X=[], y=self.data)
    ...
    def test_incremental_variance(self):
        ...
        for _ in range(1, 10):
            ...
            dw = StatAnalysis(ratio=ratio, stat=var, metrics=metrics, horizon=horizon)
            var_function_result = dw.transform(X=[], y=random_data)
            dw = StatAnalysis(ratio=ratio, stat='variance', metrics=metrics, horizon=horizon)
